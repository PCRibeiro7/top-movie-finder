...
export default async function omdbTryHardRequest(movieTitle){
    ...
    await fetch(`https://www.omdbapi.com/?s=${movieTitleArray[0]}&page=${page}&apikey=${api_key}`)
    ...
    while (failedTries<5 && failedTries>0 && finalResponse==="" ){
        let longerMovieTitle = getLongerMovieTitle(movieTitleArray,failedTries)
        await fetch(`https://www.omdbapi.com/?s=${longerMovieTitle}&page=${page}&apikey=${api_key}`)
        .then(function(response) {
            return response.json();
        })
